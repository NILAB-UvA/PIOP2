...
def _run_parallel(log):
    ...
    if op.isfile(log.replace('.txt', '.tsv.gz')):
    ...
    phlog = PhilipsPhysioLog(f=log, tr=tr, n_dyns=vols, sf=496, manually_stopped=manual_stop)  # init
    phlog.load()
    try:
...
logs = sorted(glob('../../sub-*/func/*_physio.txt'))
